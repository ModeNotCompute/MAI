...
### v8.2 (Current) — May 28, 2026
- **CRITICAL BUG FIX: Domain transfer lock detection** — RDAP API returns statuses with spaces ("client transfer prohibited") but the lock check was matching against camelCase with no spaces ("clienttransferprohibited"); fix normalizes the status string by removing all whitespace before comparison; affects WHOIS section lock display, Migration Contacts lock warning, and Maturity Scoring lock signal
- Added `clientrenewprohibited` and `serverrenewprohibited` to lock indicator list
- **Transfer Readiness guidance added** — when domain is locked, the Transfer Lock row now shows an actionable yellow guidance box explaining what needs to happen before transfer: which locks need to be removed, what each lock type means for the transfer process, recommended timing (1-2 weeks before planned transfer), and Auth/EPP code expiry windows (5-14 days)
- Guidance is context-aware: only shows bullets for the specific lock types detected (transfer, delete, update, renew)

### v8.1 — May 28, 2026
- **Custom/3rd Party Applications: path pattern matching tightened** — previously required a trailing slash (e.g., `/login/`), which missed URLs like `/login?ReturnUrl=...` (Zywave) or `/login` at end-of-path; matching now strips query strings before checking and matches patterns both with and without trailing slashes; affects all 19 path patterns (/portal, /login, /signin, /dashboard, /account, /client, /secure, /admin, /sso, /oauth, etc.)

### v8.0 — May 28, 2026
- **Manual SSL data now persists visually** in the SSL section after recalculation — previously, entering cert data and clicking "Apply & Recalculate Maturity" updated the maturity score but the SSL section still showed CT log data or "Not available"; now displays a green "MANUALLY VERIFIED" banner with issuer, expiry date (with countdown tag), and SAN count as primary SSL data; CT log data demoted to "CT Log Reference" sub-section; header badge shows "Valid" (not "Likely Valid") when manually verified; all manual data renders in PDF output
- **Migration Contacts redesigned** with separate labeled boxes — DOMAIN REGISTRAR (cyan box, large bold name, unlock/EPP action) and DNS HOST (purple box, large bold name, zone file export action) — each with uppercase label, provider name at 0.95rem bold, specific action items, and login URL where detectable; same-entity vs different-entity now shown as a clear green/yellow summary line
- **MX Record(s) row added to Email Security** — shows MX record count with green "Found" tag, full MX nameserver(s) with priority values in a code-style box; appears before Email Provider for logical flow; shows red "NOT FOUND" if no MX records exist
